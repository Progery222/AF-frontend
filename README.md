...
nginx :80  (af-frontend)
   ...
   └── /api/bulk/*    → bulk-proxy :8081 (localhost в контейнере)
                            ├── POST /orch      — параллельные вызовы orchestrator
...
| `/feed` | Свайпы и тап по координатам | `POST …/swipe`, `POST …/tap` |
...
| `BULK_MAX_CONCURRENCY` | `64` | Параллельность bulk в dev |
...
| `BULK_MAX_CONCURRENCY` | `64` | Параллельность bulk |
| `BULK_TIMEOUT_SEC` | `120` | Таймаут одного bulk-запроса |
