...
nginx :80  (af-frontend)
   ...
   ├── /api/behavior/* → behavior-engine:19096
...
Применяется на страницах: **Управление**, **FSM**, **Экран**, **Контент**, **Видео**. Координаты жестов масштабируются под разрешение каждого телефона (эталон 1080×1920).

### Управление (`/controls`)

- Системные кнопки: Домой, Назад, Последние, Power.
- Лента: свайп вверх/вниз (следующий/предыдущий пост), **свайп влево/вправо**, тап по центру.
- **Тап по координатам** — поля X/Y в эталоне 1080×1920, масштабирование на каждый телефон.

### Соцсети (`/social`)

Прокси `/api/behavior` → **behavior-engine** (`POST /social/{network}/{action}`):

| Действие | Endpoint |
|----------|----------|
| Открыть / закрыть приложение | `launch`, `close` |
| Вкладка (reels, search, …) | `open-tab` |
| Листать ленту | `feed` |
| Поиск + просмотр | `search-feed` |
| Комментарий / AI-чат | `comment`, `chat` |

Статус асинхронных job — `GET /jobs/{id}`. Соцсети: Instagram, TikTok, YouTube.

### Сценарии (`/scenarios`)

- Просмотр и редактирование `scenario.yaml` / `variables.yaml` per phone.
- ИИ-генерация черновика с автовалидацией и нормализацией YAML.
- Ручной запуск шага (`run-step`), trigger, логи JSONL, активный сценарий.
...
| `/scenarios` | YAML-сценарии: ИИ-черновик, валидация, run-step, логи | orchestrator → scenarios-engine |
| `/apps` | Системные приложения: open/close | `GET …/apps`, `POST …/apps/open\|close` |
| `/social` | Соцсети через behavior-engine (TikTok/IG/YT) | `/api/behavior/social/…` |
| `/status` | Статистика фермы | `GET /stats`, `GET /ready` |
| `/controls` | Системные кнопки + жесты ленты (свайпы, тап по координатам) | `POST …/key`, `…/swipe`, `…/tap` |
| `/feed` | Редирект на `/controls` | — |
...
| `VITE_BEHAVIOR_PROXY_TARGET` | `http://127.0.0.1:19096` | Upstream behavior-engine |
| `VITE_ORCH_API` | `/api/orch` | Base path API |
| `VITE_PROV_API` | `/api/prov` | Base path API |
| `VITE_MINIO_API` | `/api/minio` | Base path API |
| `VITE_BEHAVIOR_API` | `/api/behavior` | Base path behavior-engine |
...
AF-frontend/
├── src/
│   ├── api/           — orchestrator, provisioner, behavior-engine
│   ├── components/    — UI, LivePreviewColumn, PhoneScreenPreview, ConfirmDialog
│   ├── hooks/         — useLivePreviewTargets, useBulkAction
│   ├── lib/           — auth, phoneSort, feedGestures, runOnPhones (bulk)
│   ├── pages/         — Dashboard, Phones, Scenarios, Social, Controls, …
...
| [AF-scenarios](../AF-scenarios/README.md) | Сценарии, планировщик, ИИ-генерация |
| [AF-behavior-engine](../AF-behavior-engine/README.md) | Соцсети: launch, feed, search-feed |
